...
class S3InheritAttachment(models.Model):
	...
	def _get_S3_bucket(self, storage):
		access_key_id, secret_key, bucket_name, do_space_url, self._s3_encryption_enabled = s3_tool.parse_bucket_url(
			storage)
	...
	def _file_read(self, fname, bin_size=False):
		...
		if storage[:5] == 's3://':
			...
			try:
				...
				# super(S3InheritAttachment, self)._file_read(fname, bin_size)
		else:
			read = super(S3InheritAttachment, self)._file_read(fname, bin_size)
		return read
		...
		if storage[:5] == 's3://':
			...
			try:
				if self._s3_encryption_enabled:
					...
					self._s3_bucket.Object(s3path).put(Body=bin_value)
			except Exception as e:
				_logger.error("--File Write Exception as e: %r--",e)
		else:
			fname = super(S3InheritAttachment, self)._file_write(value, checksum)
		return fname
	...
	def _file_gc(self):
		...
		if storage[:5] == 's3://':
			...
			try:
				...
				# return super(S3Attachment, self)._file_gc(fname)
			cr.commit()
			_logger.debug("S3: filestore gc %d checked, %d removed", len(checklist), removed)
		else:
			return super(S3InheritAttachment, self)._file_gc()
